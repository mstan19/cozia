...
const AddProductForm = () => {
	...
	const onSubmit = async (event) => {
		...
		try {
			...
			let oneCategory = categoryData.categories.find(
				(item) => item.name === finalFormProductData.category
			);
			let categoryID = oneCategory?._id ? oneCategory._id : "";
			finalFormProductData["price"] = parseFloat(
				finalFormProductData.price
			);
			finalFormProductData["discount"] = parseFloat(
				finalFormProductData.discount
			);
			...
			await addProduct({
				variables: {
					productData: finalFormProductData,
	...
	return (
		<div className="h-full w-full">
			<div className="container mx-auto w-full pt-0 md:w-[44rem]">
				<div
					className="registerStyle product-form row bg-white p-6"
					...
					<form onSubmit={onSubmit} className="p-0 m-0">
						...
						<div className="grid grid-cols-3 gap-4 mb-4">
							...
							<div className="relative mb-4">
								...
								<input
									className="appearance-none border border-black w-full py-2 pl-11 lg:px-11 text-black-700 leading-tight"
									name="price"
									id="price"
									type="number"
									placeholder="0.00"
