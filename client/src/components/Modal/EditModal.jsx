...
const EditModal = ({ setEditOpenModal, onEditProduct, onEditFunction }) => {
	...
	const submitHandler = async (event) => {
		...
		try {
			...
			finalProductData["discount"] = parseFloat(finalProductData.discount);
			finalProductData["countInStock"] = parseFloat(finalProductData.countInStock);
			delete finalProductData["category"];
			const updatedProducts = await editProduct({
				variables: { productId: onEditProduct._id, productsByCategory: oneCategoryData.getCategory._id, productData: finalProductData },
			});
				...
				<div className="relative w-auto md:ml-60 mx:auto w-full pt-0 md:w-[44rem]">
					<div className="border-0 shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
						...
						<div className="relative p-3 flex-auto">
							<div className="block text-black text-lg ">To edit this product, please change the following fields.</div>
						...
						<div className="relative flex-auto">
							...
							<div className="mb-4">
								...
								<input {...register("productName")} className="appearance-none border border-black w-full py-2 px-3 text-black-700 leading-tight " name="productName" type="text" onChange={handleInputChange} ></input>
							</div>
								{error &&
							...
							<textarea rows="5" {...register("description")} className="block appearance-none border border-black w-full p-2.5 px-3 text-black-700 leading-tight" name="description" type="text" onChange={handleInputChange}></textarea>
						</div>
						<div className="grid grid-cols-3 gap-4 mb-4">
							...
							<div className="col-span-2">
								...
								<div className="inline-block relative w-full">
									<input {...register("color")} className="w-full block appearance-none bg-white border border-black hover:border-black px-4 py-2 pr-8 rounded leading-tight focus:outline-none" name="color" type="text" onChange={handleInputChange} ></input>
								</div>
								{error &&
									error?.graphQLErrors[0]?.extensions?.exception?.errors?.hasOwnProperty(
										"color"
						...
						<div className="grid grid-cols-3 gap-4 mb-4">
							<div>
								...
								<input {...register("countInStock")} className="appearance-none border border-black w-full py-2 px-3 text-black-700 leading-tight " name="countInStock" type="text" onChange={handleInputChange} ></input>
							</div>
							<div className="relative mb-4">
								...
								<input {...register("price")} className="appearance-none border border-black w-full py-2 px-11 text-black-700 leading-tight" name="price" type="number" placeholder="0.00" onChange={handleInputChange} ></input>
								{error &&
									error?.graphQLErrors[0]?.extensions?.exception?.errors?.hasOwnProperty(
										"price"
									) ? (
							...
							<div className="relative mb-4">
								...
								<input {...register("discount")} className="appearance-none border border-black w-full py-2 px-4 text-black-700 leading-tight" name="discount" type="number" onChange={handleInputChange} ></input>
							</div>
						</div>
						<div className="">
							...
							<input {...register("image")} className="appearance-none border border-black w-full py-2 px-3 text-black-700 leading-tight" name="image" type="url" onChange={handleInputChange} ></input>
							{error &&
								error?.graphQLErrors[0]?.extensions?.exception?.errors?.hasOwnProperty(
									"image"
								) ? (
